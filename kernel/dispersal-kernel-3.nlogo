...
to go
  ...
  show timer
  let mx-n max [ln number] of patches with [number > 0]
  ask patches [
    set pcolor scale-color green ln (number + 1) mx-n 0
  ]
end
...
to setup-kernel
  ...
  set kernel-weights map [ xy -> sum map [ p -> [number] of p ] patches-at-coords eight-fold xy ] kernel-offsets
  let total-w sum kernel-weights
  set kernel-weights map [ w -> w / total-w / 8 ] kernel-weights
...
to simulate-kernel
  repeat num-dispersals [
    ask turtle 0 [
;      setxy random-float 1 - 0.5 random-float 1 - 0.5
      let tgt patch-at-heading-and-distance random-float 360 random-exponential mean-d
      if tgt != nobody [
        ask tgt [
          set number number + 1
        ]
...
to-report patches-at-coords [L]
  report map [xy -> patch item 0 xy item 1 xy] L
end
...
378
mean distance
sum [distance patch 0 0 * number] of patches / sum [number] of patches
4
1
11
...
103
show-kernel-extent
foreach kernel-offsets [xy -> foreach patches-at-coords eight-fold xy [p -> ask p [ set pcolor red ]]]
NIL
1
T
OBSERVER

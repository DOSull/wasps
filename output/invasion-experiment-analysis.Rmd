...
```{r}
ggplot(wasps.sel, aes(x=X.step., y=prop.occupied, color=as.factor(r.mean))) + 
...
ggplot(wasps.t, aes(x=lambda.1, y=X.step.)) + 
  geom_jitter(aes(color=as.factor(p.ldd)), width=.05, height=0) +
...
ggplot(wasps.t, aes(x=lambda.1, y=X.step., colour=as.factor(p.ldd))) + 
  ...
  geom_smooth(aes(group=p.ldd), colour='gray', method='lm') +
  geom_point(size=0.5) +

...
+ Even extended control programs if not pursued to eradication (which is very difficult) are no more successful than a 'release-once-and-forget' strategy (`period == 'None'`)
...
+ Importantly, even `period == 'None'` scenarios, i.e. 'release-once-and-forget` achieve this outcome, even if more slowly (after 50-60 years).
...
wasps_s %>% 
  ...
  ggplot() +
    ...
    theme(legend.position = "bottom")
```
In the case of the spatial strategy (where releases are made every year, but only at every nth site) the results are easier to parse, but again, show how sensitive eventual success is to 'knowing when to stop'. The more intense `period == 2` scenarios ultimately see full recovery of the wild wasp population in almost all cases, while the less intense `period == 4` scenarios settle into a low wasp population long term equilibrium! A key management challenge would seem to be an ability to distinguish the situations in the top row of this plot from those in the bottom!
...
```{r}
wasps %>% filter(strategy == "temporal", period == "1", grid_resolution == "1") %>%
  ggplot() +
    geom_line(aes(x = t, y = wild_t, colour = as.factor(per_site),
                  group = run_number)) +
    scale_colour_viridis_d() +
...
In this plot, the anomalous result that eradication may occur at 80 releases per site under the `spatial` strategy at `period == 2` but not at the higher release numbers is particularly obvious. This result might be due to the strain that adding so many wasps places on local habit capacity in the year of release. This could cause a local population to collapse negating the impact of the large number of GM wasps released. Understanding this result fully probably requires closer analysis of local population dynamics under different combinations of local wild + released GM wasps relative to habitat capacity --- and would require a more extensive set of experiments where the `per_site` parameter was set to a larger number of possible values.

...
+ The number of GM wasps released (`per_site`) only marginally increases the chances of success; the main effect of releasing more wasps is to accelerate the initial decrease in wasp population
...
Clean up all the NetLogo variable name weirdness with `janitor::clean_names()`, along with a couple of other tidy ups to simplify names.
...
start_pops <- wasps %>%
  ...
  rename(pop_0 = total_pop, wild_0 = total_wild, 
         gm_0 = total_gm, sterile_0 = total_sterile)
...
# Eradication success
First we tag scenarios where eradication has occurred. These are any that end 'prematurely' before the full 150 year run time elapsed.
...
wasps %>% 
  ...
  ggplot() +
    geom_tile(aes(x = grid_resolution, y = periodicity, 
                  fill = success)) + 
    scale_fill_distiller(palette = "Greens", direction = 1) + 
    facet_grid(per_site ~ strategy + program_duration, as.table = FALSE,
               labeller = label_both) +
    ...
    labs(x = "Release site grid resolution, km", y = "Periodicity", fill = "Success")
```
...
+ `spatial` (the two leftmost columns) where `periodicity` denotes that the chosen grid of release sites is activated every year, but only at every nth site; or
+ `temporal` (the two rightmost columns) where every site in the grid of release sites is activated every nth year, with n the `periodicity`.

Additionally, each strategy may be in place for either 20 or 30 years (`program_duration`). **Note** that `periodicity == 0` in both cases is a 'release-once-and-forget' strategy, which never achieves eradication in either case, and where `periodicity` and `program_duration` settings are irrelevant.

From the above plots, overall the `temporal` strategies are more successful. However in both cases, eradication only occur when `grid_resolution` is 1, i.e. a very dense network of release sites is required. `spatial` strategies furthermore only rarely achieve eradication when the `periodicity` is other than 1 (i.e. every site). This suggests that the key determinant of eradication is 'flooding' the entire landscape regularly, which prevents wild populations 'escaping' from GM wasps.

In the `temporal` strategies there is an increased chance of eradication when more colonies are released per site, such that even if releases only occur every 4 years eradication is still possible at high levels of releases. Note that these are very high levels of release --- close to half the habitat capacity of wasps!
...
ggplot(wasps) +
  ...
  facet_grid(grid_resolution ~ periodicity, scales = "free_x", labeller = label_both) +
...
+ The finding that `grid_resolution == 1`, `periodicity == 1` combinations are 100% successful at eradication is confirmed. 
...
**Remember**, this is with reference only to those scenario settings where eradication is _almost_ achieved, i.e. when `grid_resolution == 1` and `periodicity != 1`.

```{r}
wasps_t %>% 
  filter(periodicity %in% c(2, 4), grid_resolution == 1) %>%
  ggplot() +
    geom_line(aes(x = t, y = wild_t, colour = program_duration,
                  group = run_number)) +
    scale_colour_brewer(palette = "Set1") +
    facet_grid(periodicity ~ per_site, labeller = label_both) +
    labs(x = "Time, years", y = "Wild population relative to start") +
    theme(legend.position = "bottom")
```
...
wasps %>% filter(eradication == TRUE) %>%
  select(per_site, max_t, periodicity, strategy) %>%
  mutate(per_site = as.factor(per_site)) %>%
  ggplot() +
    geom_boxplot(aes(x = per_site, y = max_t, group = per_site)) +
    facet_grid(periodicity ~ strategy, labeller = label_both) + 
